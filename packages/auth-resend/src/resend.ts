 ...
 * `HttpClient`. Reads `RESEND_API_KEY` (required) and `RESEND_FROM` (defaults
 * to a scaffold address) from the environment.
 *
 * The layer fails with `ConfigError` when the API key is absent — an app
 * catches that and falls back to a development mailer.
 */
export const ResendMailerLayer: Layer.Layer<Mailer, Config.ConfigError, HttpClient.HttpClient> = Layer.effect(
  Mailer,
  Effect.gen(function*() {
